...
public class ResolveGenericsNew extends TwoPassTransformation {
	...
	public ResolveGenericsNew(CrossReferenceServiceConfiguration sc, List<CompilationUnit> cus) {
		super(sc);
		this.cus = cus;
	}
	...
	public ProblemReport analyze() {
		...
		while (fw.next()) {
			...
			if (pe instanceof VariableReference) {
				...
				if (hit || tp != null) {
					castAdded = checkCastRequired(vr, t, Util.getRequiredContextType(getSourceInfo(), vr));
				}
				...
				TypeParameter tp = isBaseTypeParameterOrArrayThereof(t);
				if (tp != null ) {
					Type req = Util.getRequiredContextType(si, (Expression)mr);
				...
				int i = 
					mr instanceof MethodReference ? 
							si.getMethods(m.getContainingClassType(), m.getName(), transSig).size()
						:	si.getConstructors(m.getContainingClassType(), transSig).size();
				if (i > 1) {
			...
			if (!castAdded 
				...
				Type t = Util.getRequiredContextType(si, (Expression)pe);
		...
		for (int i = 0; i < mrs.size(); i++) {
			...
			if (methodReferenceResolved instanceof ResolvedGenericMethod) {
				...
				if (crs.getType(mr) == null)
					...
					tr = TypeKit.createTypeReference(crs, crs.getType(mr), mr); 
	...
	private ClassType getFirstBound(TypeParameter tp) {
		ClassType res = tp;
		while (res instanceof TypeParameter) {
			res = getNameInfo().getClassType(tp.getBoundName(0));
		}
		return res;
	}
	...
	public void transform() {
		...
		for (int i = guardMethods.size()-1; i >= 0; i--) {
			...
			ProgramFactory f = getProgramFactory();
			ASTList<Expression> args = mr.getArguments().deepClone();
			if (tgt != null)
				args.add(0, tgt);
