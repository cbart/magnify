 ...
 *
 */
public class ReplaceEnumsNew extends TwoPassTransformation {
	...
	private ClassDeclaration makeReplacement(EnumDeclaration ed) {
		...
		for (MemberDeclaration md: ed.getMembers()) {
			if (md instanceof ClassInitializer) {
				...
				members.add(makeReplacement((EnumDeclaration)md));
	...
	private void runRetrotranslator() {
		...
		while (st.hasMoreTokens()) {
			...
			if (s.replace("/", "\\").startsWith(FileUtils.getPathOfExtensionClassesDir().getAbsolutePath().replace("/", "\\"))
					|| s.replace("/","\\").startsWith(FileUtils.getPathOfSystemClasses().getAbsolutePath().replace("/", "\\"))) {
			...
			Retrotranslator.main(new String[] {
					jar ? "-srcjar" : "-srcdir", s,
