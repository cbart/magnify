		...
		for (CompilationUnit cu : cul) {
			...
			while (tw.next()) {
				...
				if (pe instanceof TypeDeclaration)
					currentType = (TypeDeclaration)pe;
...
//				MethodReference mr = getProgramFactory().createMethodReference(urq, old.getIdentifier(), old.getArguments(), old.getTypeArguments());
//				replace(old, mr);
...
//				TypeReference tr = MiscKit.createUncollatedReferenceQualifier(getProgramFactory(), x).toTypeReference();
//				replace(hs.r, tr);
				...
				getChangeHistory().attached(tr);
//				UncollatedReferenceQualifier tr = MiscKit.createUncollatedReferenceQualifier(getProgramFactory(), x);
//				replace(hs.r, tr);
			}
		}
	}
