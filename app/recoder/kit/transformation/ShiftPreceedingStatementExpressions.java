 ...
 * during execution of the resulting statement, while preserving the behavior.
 * The method changes all statement expressions (which might have side-effects)
 * that are evaluated before the given expression into initializations of
 * temporary variables. These will preceed the statement the given expression is
 * located within. If the expression is contained in a statement, a statement
 * block is inserted if needed. If the expression is part of a field
 ...
 * initializer block is added. Otherwise, if the expression is in a statement
 * that cannot be prepended by variable declarations and has preceeding
 * expressions with side-effects, the statement is embedded in a new statement
 * block by {@link recoder.kit.transformation.PrepareStatementList}.
 * Afterwards, or if this has not been necessary, a new temporary variable
 * declaration is added as statement in front of the containing statement, and a
 * new variable reference replaces the old expressions.
 * <DT>Removed:
 * <DD>If the expression was in a field initializer, the initializer is cloned
 * and replaced (the clone becomes a copy assignment within the new initializer
 * block). Otherwise, the parent statement contained in a statement container
 * might be cloned and embedded in a statement block. All preceeding expressions
...
public class ShiftPreceedingStatementExpressions extends TwoPassTransformation {
     ...
     * with side-effects (statement expressions).
     */
    private List<Expression> preceeding;

    /**
     * The temporary variable declarations to be inserted. They will contain
     * clones of the preceeding expressions.
     */
    private List<Statement> tempVarDecls;

    /**
     * The references to the new temporary variables to replace the preceeding
     * expressions.
     */
    ...
    /**
     * The new parent of the expression; the old parent if it was a statement,
     * or a copy assignment if it was a field specification - the initialization
     * must move to the initializer block.
     */
    ...
    /**
     * Finds out which expressions have to be shifted and their types, creates
    ...
    public ProblemReport analyze() {
        ...
        preceeding = collectPreceeding(expression);
         ...
         * parent.getASTParent() instanceof recoder.java.statement.For) { parent =
         * parent.getASTParent(); }
         */
        ...
        if (exSize > 0) {
            ...
            for (int i = 0; i < exSize; i += 1) {
                ...
                LocalVariableDeclaration vdecl = f.createLocalVariableDeclaration(minTypeRef, f
    ...
    private List<Expression> collectPreceeding(Expression e) {
        ...
        for (int i = exSize - 1; i >= 0; i -= 1) {
            if (!ExpressionKit.containsStatements(res.get(i))) {
                res.remove(i);
            }
        }
        return res;
    ...
    public void transform() {
        ...
        if (parent instanceof Statement) {
            ...
            FieldDeclaration fd = fs.getParent();
    ...
    /**
     * Returns the statement in a statement container or the field specification
     * the given expression is located within. Valid after analyze.
     */
    ...
    /**
     * After the transformation, returns the statement that contains the given
