...
public interface SourceFileRepository extends Service {
    ...
    /**
     * Returns the list of compilation unit in the project. This call may
     * trigger an update.
     */
    ...
    /**
     * Returns the location of the source file for the given class or
    ...
    /**
     * Retrieves the AST for the compilation unit containing the class with the
     * given name. This method will not throw ParserExceptions, but pass these
     * to the system error handler.
     * 
     * @param classname
    ...
    /**
     * Retrieves the abstract syntax tree for the compilation unit within the
     * specified file. The service guarantees not to retrieve two different
     * syntax trees for the same location. The given filename may also be
     * absolute - it will then be shortened if a prefix is in the search path,
     * or used to load the file regardless of the search path, otherwise.
     * 
     * @param filename
    ...
    /**
     * Retrieves all compilation units from the current search path,
     * transitively. This is useful to create a closed world, but assumes that
     * all subdirectories of the search path belong to the project.
     * 
    ...
    /**
     * Retrieves compilation units from the current search path, as given by the
     * filter. The algorithm checks all files in all subdirectories, but this
     * can be narrowed down in the filter.
     * 
     * @param filter
     ...
     * after the last persistant state.
     * 
     * @param cu
     *            a compilation unit.
    ...
    /**
     * Print back the compilation unit to it's data location. If the compilation
     * unit is still at its original location, apply the filename mapper.
     */
    void print(CompilationUnit cu) throws IOException;

    /**
     * Print back all compilation units to their data locations. Can be used to
     * print all units, or only those that are not up to date.
     * 
     * @param always
     *            flag to indicate whether all units should be written back (
